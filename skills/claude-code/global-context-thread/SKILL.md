...
description: 在调用链、影响面、模块关系或复杂工作流关系确实需要结构化事实源时，路由到 context-thread 或 .Ai-config 关系索引；它只辅助关系定位，不主导 AI 配置修复或普通局部任务。
when_to_use: 用户询问调用链、影响面、模块关系、复杂配置关系、跨文档工作流、任务依赖、多人或多 AI 接手，且结构化查询能减少大范围 grep/read 时使用；简单问答、小修小补、明确局部文件任务、纯 Markdown/任务卡状态分析不使用。
---
