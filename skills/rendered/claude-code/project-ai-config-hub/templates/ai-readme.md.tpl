...
如果存在 `docs/ai/CURRENT.md`，在任务有接手价值、可能跨会话、涉及多任务切换或属于高风险修改时先读取它，判断是否有当前活动任务和对应任务卡。简单问答、一次性命令和一轮内完成的小修复，不需要启动完整状态流程。
...
- `docs/ai/tasks/*.md` 用于保存跨会话、被打断、等待确认、阻塞或有残留风险任务的目标、上下文、已尝试/已排除、验证、风险和下一步。
