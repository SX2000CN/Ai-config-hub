...
description: 创建、重设计或审查非简单前端界面时，作为实现领域 skill：Design Read、product/marketing 双轨、三旋钮与一个 signature，落地可维护、可访问、响应式且状态完整的 UI；设计先行用短 UI brief，不依赖 Pencil；局部 UI bug fix、小样式修复、已有设计直接实现、后端/文档任务不使用。
---
