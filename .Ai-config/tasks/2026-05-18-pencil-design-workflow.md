...
- 2026-05-25 已将 Pencil MCP 配置升级为 `sync-mcp.ps1` 的本机同步动作；晚间实测确认 Desktop transport 仍不可用而 VS Code 插件端可用，因此同步时只自动发现插件端 MCP server，并把既有 Desktop 配置替换为 VS Code / Cursor 等插件端。
...
- 本机 `scripts\mcp-local.ps1` 能发现 `C:\Users\sx200\.pencil\mcp\visual_studio_code\out\mcp-server-windows-x64.exe`，默认 app 为 `visual_studio_code`；如设置 `AI_CONFIG_HUB_PENCIL_MCP_COMMAND` / `AI_CONFIG_HUB_PENCIL_MCP_APP`，同步脚本会优先使用环境变量指定的 server。
...
原始创建和首次同步已完成，各轮规则修正均已渲染、检查并同步到本机。2026-07-08 按 `.Ai-config/CURRENT.md` 形态约束的 30 天未跟进规则标记关闭：功能已交付并同步超过 30 天无用户反馈，视为验证通过；如后续实际使用中发现问题，重新开卡跟进。
