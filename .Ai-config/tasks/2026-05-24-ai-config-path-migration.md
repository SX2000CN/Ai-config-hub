...
迁移已完成，旧版 `docs/ai/` 已清理，`.Ai-config/` 作为当前项目 AI 配置中枢已稳定运行超过 30 天，期间未发现路径引用问题。2026-07-08 按 `.Ai-config/CURRENT.md` 形态约束的 30 天未跟进规则标记关闭；如后续发现历史路径引用问题，按具体引用修复即可，不需要重开本任务卡。
