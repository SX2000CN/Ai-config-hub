...
当前状态：v3 hygiene 已纳入全局规则与 project-ai-config-hub。CURRENT 必须保持薄仪表盘，并在「更新项目 AI 配置」时强制健康检查与修复。
...
- CURRENT 有强制 hygiene：行数/条数上限、禁区内容、关闭同步、超龄待确认滚出；详情只在任务卡或正式文档。
- 只依赖普通 Markdown 文件和轻量规则，不把 hook、数据库或脚本状态机作为核心机制。
- 支持渐进启用：小项目可以只有项目规则或 `CURRENT.md`，长期项目再补任务卡、registry 和项目级 skills。
- 用户说「更新项目 AI 配置」时，project-ai-config-hub 必须 audit+repair CURRENT 健康问题，而不是继续追加。
