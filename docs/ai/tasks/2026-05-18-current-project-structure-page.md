...
`pencil-design-workflow` 全局 skill 与浏览器视觉验证 MCP 已同步到本机用户级 Claude Code / Codex 配置。当前项目没有前端应用入口，也没有项目级 `.claude/skills`、`.agents/skills` 或 `.codex/skills` 入口；本仓库主要维护全局 rules、skills、MCP 配置片段和 `docs/ai` 工作状态。
...
- `docs/ai/CURRENT.md` 当前可作为任务状态入口。
- `docs/ai/skills-registry.md` 登记的是本仓库维护的全局 skill 源，不是需要生成到 `.claude/skills` 的项目级 skill。
