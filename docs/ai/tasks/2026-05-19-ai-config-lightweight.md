...
在不损失护栏效果的前提下，把全局 AI 配置和项目级 `docs/ai` 机制改成按风险、接手价值渐进启用，减少日常使用中的流程负担，同时保留模型主动发挥和必要安全边界。
...
- 若日常使用仍感觉偏重，下一轮应优先微调 `docs/work-state-design.md` 和 `docs/ai/tasks/README.md` 中偏保守的状态语气。
...
- `docs/ai/CURRENT.md`：本仓库当前状态总览。

## 不要重复

- 不要把简单任务重新强制纳入任务卡流程。
- 不要把完整 `docs/ai/tasks/`、`archive/`、`skills-registry.md` 当作所有项目的默认健康指标。

## 关闭依据 / 最终结果

已完成仓库源文件、rendered 产物和本机全局配置同步；等待用户日常试用后确认是否关闭或继续微调。
