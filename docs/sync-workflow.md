...
文件使用原子替换，目录使用 staging/backup 目录切换；同一管线中任一步失败时，脚本按逆序恢复本次已经更新的目标并保留历史备份。MCP 合并事务覆盖 Claude / Codex / Grok；可识别的 retired pencil 配置在 Apply 时安全移除。runtime 只有在 staging 安装和 smoke check 通过后才切换。
...
- 完整 `C:\Users\sx200\.codex\config.toml` / `C:\Users\sx200\.grok\config.toml` 不作为仓库事实源；只有 MCP 配置片段流程会合并明确托管的 server section（Grok 另含 managed compat 块）。
...
- `tool-configs/mcp/shared/local-webfetch.json`：交付 Claude Code 与 Grok。
- `tool-configs/mcp/shared/context-thread.json`：Claude Code / Codex / Grok 共用。
...
| Profile | 能力 |
|---|---|
| `core` | Claude Code 与 Grok 的 local-webfetch；Codex 不注册 managed MCP |
| `code-intel` | core + context-thread |
| `browser` | core + Playwright（Grok 默认 headless） |
| `browser-debug` | core + Chrome DevTools |
| `design` | core 兼容别名（原 Pencil 设计面已退役） |
| `full` | 四 managed server；仅临时全开，勿日常常驻 |
...
- 不要提交完整 `~\.claude.json`、`~\.codex\config.toml` 或 `~\.grok\config.toml`。
...
- 可识别的 retired `pencil` MCP（路径/参数含 Pencil 特征）在 Apply 时从 Claude/Codex/Grok 配置安全移除；无法识别的同名自定义 server 保留并提示。
