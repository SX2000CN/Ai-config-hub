...
- 三套 runtime、规则、skills 和六个 MCP profile 的用户级 dry-run。
...
文件使用原子替换，目录使用 staging/backup 目录切换；同一管线中任一步失败时，脚本按逆序恢复本次已经更新的目标并保留历史备份。Claude Code MCP 合并和 Pencil CLI 注册属于同一事务，最终配置验证失败时恢复原始用户配置；两个 runtime 也只有在 staging 安装和 smoke check 通过后才切换。
...
- `sync-skills.ps1 -Apply` 只覆盖带对应 `ai-config-hub-managed` 标记的托管目录；同名但无标记的目录视为用户资产并拒绝覆盖，历史安装需先显式迁移。
...
- `tool-configs/mcp/shared/local-webfetch.json`：仅交付 Claude Code。
- `tool-configs/mcp/shared/context-thread.json`：Claude Code / Codex 共用。
- `tool-configs/mcp/shared/playwright.json`：固定使用 browser runtime 中的 `@playwright/mcp@0.0.78`。
- `tool-configs/mcp/shared/chrome-devtools.json`：固定使用 browser runtime 中的 `chrome-devtools-mcp@1.6.0`。
- `pencil` 不写静态 JSON，由 `scripts/mcp-local.ps1` 发现 VS Code / Cursor 等 IDE 插件端；Desktop 只在用户明确要求诊断时处理。

Profile 固定为：

| Profile | 能力 |
|---|---|
| `core` | Claude Code 的 local-webfetch；Codex 不注册 managed MCP |
| `code-intel` | core + context-thread |
| `browser` | core + Playwright |
| `browser-debug` | core + Chrome DevTools |
| `design` | core + 本机发现的 Pencil |
| `full` | core + context-thread + 两个 browser server + Pencil |
...
`Source` 从仓库 runtime 启动离线 MCP 握手；`Readiness` 检查用户级 runtime、版本、hash 和安装漂移。可比较 hash 包含完整执行 payload、`package.json`、lockfile 和 lockfile 登记的生产依赖，日志与缓存不参与比较；因此已锁定 MCP 包被修改或缺失时会判定未就绪。`Smoke` 在 Readiness 基础上对已安装 runtime 执行 entry probe、离线 initialize + `tools/list` 和预期工具数校验。doctor 同时报告工具数量、首选路由冲突和 Pencil 发现状态。默认不做网络探测；未来任何网络探测也必须显式允许。
...
- 不要提交完整 `~\.claude.json` 或 `~\.codex\config.toml`。
...
- Codex 通过 `# >>> ai-config-hub managed mcp: <server>` marker 识别托管 block；Claude JSON 通过与已登记 current/legacy 精确配置比较确认归属。
- 无法确认归属的 Pencil 注册始终保留。选择 `design/full` 且发现 IDE 插件 MCP 时，Claude Code 通过官方 `claude mcp add -s user` 注册，Codex 合并 `[mcp_servers.pencil]`；两端和 Pencil 操作属于同一事务，最终校验失败会恢复原配置。
- 浏览器 server 不在启动时运行 `npx -y`，只通过已安装且 lockfile 精确固定的 browser runtime 启动；检查脚本拒绝 `@latest` 和 managed `npx` 回流。
