...
> 同一套核心规则应尽量同时适用于 Claude Code、Codex、Grok Build 和其他 AI 编码工具。工具专属机制放在各自补充段，不混入通用核心。
...
`CURRENT.md` 是接手导航仪表盘，不是项目百科、运维 runbook 或 changelog。每次更新 CURRENT 或切换/结束任务前，必须先做 hygiene，再写入本轮变化：

- 软上限：整文件建议不超过约 120 行或 6KB；活跃任务最多 3 条且每条最多约 4 行；待确认与暂停/阻塞各最多 5 条且必须带日期；最近关闭最多 5 条且每条一行。
- 详情只写在 `tasks/*.md` 或正式文档；CURRENT 只保留指针、状态和最小下一步。
- 禁止写入：完整启动命令、环境拓扑长文、BuildId/SHA 账本、验收日志正文、技术栈百科、changelog 正文；最多一行链接到正确文档或任务卡。
- 关闭任务卡后必须同步从 CURRENT 的活跃/待确认/暂停区移除对应行；超龄待确认（默认 14 天，项目模板另有约定从其约定）应催关闭或标为未跟进关闭并滚出。
- 用户说「更新项目 AI 配置」「修复项目 AI 配置」等时，应按 `project-ai-config-hub` 的 audit+repair 执行，包括 CURRENT hygiene、过期入口和已退役全局能力残留的修复，而不是只追加新状态。
