...
脉络 MCP 使用项目内本地引擎源码路线，不要求全局安装 `context-thread` 命令。首次使用前先构建本地引擎：
...
- 脉络 MCP 通过 `scripts/context-thread.ps1` 启动 `tools/context-thread-engine` 本地引擎构建产物，不依赖全局 `context-thread` 命令；源码随项目维护，`dist/` 和 `node_modules/` 由 `.gitignore` 忽略。
...
- `global-context-thread` 的定位是“脉络”轻量结构化事实层 skill，用来在代码关系、配置关系、影响面或复杂工作流关系分析中优先查询 context-thread 或 `docs/ai` 关系索引，同时保持 L0/L1 小任务不升级。
