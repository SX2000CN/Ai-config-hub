 ...
 *
 * Defines the tools exposed by the ContextThread MCP server.
 */

import ContextThread, { findNearestContextThreadRoot } from '../index';
...
/**
 * Node kinds that contain other symbols. For these, `context_thread_node` with
...
/**
 * Calculate the recommended number of context_thread_explore calls based on project size.
...
/**
 * Adaptive output budget for `context_thread_explore`, scaled to project size.
 *
...
/**
 * Whether `context_thread_explore` should prefix source lines with their line
 ...
 *
 * Defaults ON. Set `CONTEXT_THREAD_EXPLORE_LINENUMS=0` to disable (used by the
 * A/B harness to measure the payload-cost vs. read-savings tradeoff).
 */
function exploreLineNumbersEnabled(): boolean {
  return process.env.CONTEXT_THREAD_EXPLORE_LINENUMS !== '0';
}
 ...
 * machine any other local user can pre-create `context-thread-consulted-<hash>` as
...
function markSessionConsulted(sessionId: string): void {
  try {
    ...
    const markerPath = join(tmpdir(), `context-thread-consulted-${hash}`);
...
/**
 * All ContextThread MCP tools
 *
 * Designed for minimal context usage - use context_thread_context as the primary tool,
...
export const tools: ToolDefinition[] = [
  {
    name: 'context_thread_search',
    description: 'Quick symbol search by name. Returns locations only (no code). Use context_thread_context instead for comprehensive task context.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  {
    name: 'context_thread_context',
    description: 'PRIMARY TOOL — call this FIRST for any "how does X work", architecture, feature, or bug-context question. Composes search + node + callers + callees and returns entry points, related symbols, and key code in ONE call — usually enough to answer with no further search/Read/Grep. Prefer this over chaining context_thread_search + context_thread_node, and over context_thread_explore. NOTE: provides CODE context, not product requirements; for new features still clarify UX/edge cases with the user.',
    inputSchema: {
      type: 'object',
      properties: {
        task: {
  ...
  {
    name: 'context_thread_node',
    description: 'Get detailed info about ONE symbol (location, signature, docstring). Pass includeCode=true for source: a function/method returns its body; a class/interface/struct/enum returns a compact member OUTLINE (fields + method signatures + line numbers), not every method body — Read or context_thread_node a specific member for its body. Keep includeCode=false to minimize context. For SEVERAL related symbols, make ONE context_thread_explore (or context_thread_context) call instead of many node calls — repeated node calls each re-read the whole context and cost far more.',
    inputSchema: {
      type: 'object',
      properties: {
        symbol: {
  ...
  {
    name: 'context_thread_explore',
    description: 'Returns source for SEVERAL related symbols grouped by file, plus a relationship map, in ONE capped call. This is the efficient way to inspect many related symbols at once — strongly prefer it over a series of context_thread_node or Read calls (each separate call re-reads the whole context, so 8 node calls cost far more than 1 explore). Use it after context_thread_context when you need to see the actual source of several symbols. Query with specific symbol/file/code terms, NOT natural-language sentences — run context_thread_search first to find names. Bad: "how are agent prompts loaded and passed to the CLI". Good: "renderStaticScene drawElementOnCanvas ShapeCache renderElement.ts".',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Symbol names, file names, or short code terms to explore (e.g., "AuthService loginUser session-manager", "GraphTraverser BFS impact traversal.ts"). Use context_thread_search first to find relevant names.',
        },
        maxFiles: {
          type: 'number',
  ...
  {
    name: 'context_thread_status',
    description: 'Get the status of the ContextThread index, including statistics about indexed files, nodes, and edges.',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: projectPathProperty,
  ...
  {
    name: 'context_thread_files',
    description: 'REQUIRED for file/folder exploration. Get the project file structure from the ContextThread index. Returns a tree view of all indexed files with metadata (language, symbol count). Much faster than Glob/filesystem scanning. Use this FIRST when exploring project structure, finding files, or understanding codebase organization.',
    inputSchema: {
      type: 'object',
      properties: {
        path: {
...
/**
 * Tool handler that executes tools against a ContextThread instance
 *
 * Supports cross-project queries via the projectPath parameter.
 * Other projects are opened on-demand and cached for performance.
 */
export class ToolHandler {
  // Cache of opened ContextThread instances for cross-project queries
  private projectCache: Map<string, ContextThread> = new Map();
  // The directory the server last searched for a default project. Surfaced in
  // the "not initialized" error so users can see why detection missed.
  private defaultProjectHint: string | null = null;

  constructor(private cg: ContextThread | null) {}

  /**
   * Update the default ContextThread instance (e.g. after lazy initialization)
   */
  setDefaultContextThread(cg: ContextThread): void {
    this.cg = cg;
  }
  ...
  /**
   * Whether a default ContextThread instance is available
   */
  hasDefaultContextThread(): boolean {
    return this.cg !== null;
  }

  /**
   * Get tool definitions with dynamic descriptions based on project size.
   * The context_thread_explore tool description includes a budget recommendation
  ...
  getTools(): ToolDefinition[] {
    ...
    try {
      ...
      return tools.map(tool => {
        if (tool.name === 'context_thread_explore') {
          return {
            ...tool,
  ...
  /**
   * Get ContextThread instance for a project
   *
  ...
  private getContextThread(projectPath?: string): ContextThread {
    ...
    const cg = ContextThread.openSync(resolvedRoot);
  ...
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    try {
      ...
      // The `path` and `pattern` properties used by context_thread_files are
      ...
      switch (toolName) {
        case 'context_thread_search':
          return await this.handleSearch(args);
        case 'context_thread_context':
          return await this.handleContext(args);
        case 'context_thread_callers':
          return await this.handleCallers(args);
        case 'context_thread_callees':
          return await this.handleCallees(args);
        case 'context_thread_impact':
          return await this.handleImpact(args);
        case 'context_thread_explore':
          return await this.handleExplore(args);
        case 'context_thread_node':
          return await this.handleNode(args);
        case 'context_thread_status':
          return await this.handleStatus(args);
        case 'context_thread_files':
  ...
  /**
   * Handle context_thread_explore — deep exploration in a single call
   *
   * Strategy: find relevant symbols via graph traversal, group by file,
   * then read contiguous file sections covering all symbols per file.
   * This replaces multiple context_thread_node + Read calls.
   *
  ...
  private async handleExplore(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (budget.includeCompletenessSignal) {
      ...
      lines.push('');
      lines.push(`> Some file sections were trimmed for size. Use \`context_thread_node\` or Read for the full source if needed.`);
    }
    ...
    if (output.length > budget.maxOutputChars) {
      ...
      return this.textResult(safe + '\n\n... (explore output truncated to budget — use context_thread_node or Read for more)');
    }
    return this.textResult(output);
  }

  /**
   * Handle context_thread_node
   */
  private async handleNode(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (includeCode) {
      ...
      // Read or context_thread_node a specific method for its body. Leaf symbols
  ...
  /**
   * Handle context_thread_files - get project file structure from the index
   */
  private async handleFiles(args: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (allFiles.length === 0) {
      return this.textResult('No files indexed. Run `context-thread index` first.');
    }
  ...
  private findSymbol(cg: ContextThread, symbol: string): { node: Node; note: string } | null {
   ...
   */
  private findAllSymbols(cg: ContextThread, symbol: string): { nodes: Node[]; note: string } {
   ...
   */
  private buildContainerOutline(cg: ContextThread, node: Node): string {
  ...
  private formatNodeDetails(node: Node, code: string | null, outline?: string | null): string {
    ...
    if (outline) {
      lines.push('', outline, '',
        `> Structural outline only. Read \`${node.filePath}\` or call context_thread_node on a specific member for its body.`);
    } else if (code) {
