 ...
 *
 * Model Context Protocol server that exposes ContextThread functionality
 ...
 * @example
 * ```typescript
 * import { MCPServer } from 'context-thread';
 *
...
/**
 * MCP Server for ContextThread
 *
 * Implements the Model Context Protocol to expose ContextThread
 * functionality as tools that can be called by AI assistants.
 */
export class MCPServer {
  private transport: StdioTransport;
  private cg: ContextThread | null = null;
   ...
   *
   * Note: ContextThread initialization is deferred until the initialize request
  ...
  async start(): Promise<void> {
    ...
    const pollMs = parsePpidPollMs(process.env.CONTEXT_THREAD_PPID_POLL_MS);
    if (pollMs > 0) {
      this.ppidWatchdog = setInterval(() => {
        ...
        if (ppidChanged || hostGone) {
          ...
          process.stderr.write(
            `[ContextThread MCP] Parent process exited (${reason}); shutting down.\n`
          );
          this.stop();
        }
      }, pollMs);
  ...
  private async tryInitializeDefault(projectPath: string): Promise<void> {
    ...
    try {
      this.cg = await ContextThread.open(resolvedRoot);
      this.toolHandler.setDefaultContextThread(this.cg);
      this.startWatching();
    } catch (err) {
      // Log the error so transient failures are diagnosable (see issue #47)
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[ContextThread MCP] Failed to open project at ${resolvedRoot}: ${msg}\n`);
    }
  }
  ...
  private async retryInitIfNeeded(): Promise<void> {
    ...
    if (this.toolHandler.hasDefaultContextThread()) return;
    ...
    const resolvedRoot = findNearestContextThreadRoot(candidate);
    ...
    try {
      ...
      this.cg = ContextThread.openSync(resolvedRoot);
      this.projectPath = resolvedRoot;
      this.toolHandler.setDefaultContextThread(this.cg);
      this.startWatching();
    } catch {
  ...
  private async initFromRoots(): Promise<void> {
    ...
    try {
      ...
      if (rootPath) {
        ...
        process.stderr.write('[ContextThread MCP] Client returned no workspace roots; falling back to process cwd.\n');
      }
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`[ContextThread MCP] roots/list request failed (${msg}); falling back to process cwd.\n`);
    }
    await this.tryInitializeDefault(target);
  }

  /**
   * Start file watching on the active ContextThread instance.
  ...
  private startWatching(): void {
    ...
    // CONTEXT_THREAD_NO_WATCH=1), say so explicitly and tell the user how to keep
    ...
    if (disabledReason) {
      process.stderr.write(
        `[ContextThread MCP] File watcher disabled — ${disabledReason}. ` +
        `The graph will not auto-update; run \`context-thread sync\` (or install the git sync hooks via \`context-thread init\`) to refresh.\n`
      );
      return;
    }
    ...
    const started = this.cg.watch({
      onSyncComplete: (result) => {
        if (result.filesChanged > 0) {
          process.stderr.write(
            `[ContextThread MCP] Auto-synced ${result.filesChanged} file(s) in ${result.durationMs}ms\n`
          );
        }
      },
      onSyncError: (err) => {
        process.stderr.write(`[ContextThread MCP] Auto-sync error: ${err.message}\n`);
      },
    });

    if (started) {
      process.stderr.write('[ContextThread MCP] File watcher active — graph will auto-sync on changes\n');
    } else {
      // start() can also return false when recursive fs.watch isn't supported.
      process.stderr.write(
        '[ContextThread MCP] File watcher unavailable on this platform — run `context-thread sync` to refresh the graph after changes.\n'
      );
    }
  }
  ...
  stop(): void {
    ...
    // Close the main ContextThread instance
    if (this.cg) {
      this.cg.close();
      this.cg = null;
    }
  ...
  private async handleToolsCall(request: JsonRpcRequest): Promise<void> {
    ...
    // initialized after the MCP server started (e.g. user ran context-thread init)
    await this.retryInitIfNeeded();
