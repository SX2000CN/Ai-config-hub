 ...
 * ContextThread CLI
 *
 * Command-line interface for ContextThread code intelligence.
 *
 * Usage:
 *   context-thread                    Show CLI help
 *   context-thread init [path]        Initialize ContextThread in a project
 *   context-thread uninit [path]      Remove ContextThread from a project
 *   context-thread index [path]       Index all files in the project
 *   context-thread sync [path]        Sync changes since last index
 *   context-thread status [path]      Show index status
 *   context-thread query <search>     Search for symbols
 *   context-thread files [options]    Show project file structure
 *   context-thread context <task>     Build context for a task
 *   context-thread callers <symbol>   Find what calls a function/method
 *   context-thread callees <symbol>   Find what a function/method calls
 *   context-thread impact <symbol>    Analyze what code is affected by changing a symbol
 *   context-thread affected [files]   Find test files affected by changes
 */

import { Command } from 'commander';
import * as path from 'path';
import * as fs from 'fs';
import { getContextThreadDir, isInitialized } from '../directory';
...
async function loadContextThread(): Promise<typeof import('../index')> {
  try {
    ...
    console.error(`\x1b[31m${getGlyphs().err}\x1b[0m Failed to load ContextThread modules.`);
    console.error(`\n  Node: ${process.version}  Platform: ${process.platform} ${process.arch}`);
    console.error(`\n  Error: ${msg}`);
    console.error('\n  Rebuild the local engine with: .\\scripts\\context-thread.ps1 bootstrap\n');
    process.exit(1);
  }
}
...
// Block ContextThread on Node.js 25.x — V8's turboshaft WASM JIT has a Zone
...
process.on('uncaughtException', (error) => {
  console.error('[ContextThread] Uncaught exception:', error);
});

process.on('unhandledRejection', (reason) => {
  console.error('[ContextThread] Unhandled rejection:', reason);
});
...
function resolveProjectPath(pathArg?: string): string {
  ...
  // If exact path is initialized (has context-thread.db), use it
...
function writeErrorLog(projectPath: string, errors: Array<{ message: string; filePath?: string; severity: string; code?: string }>): void {
  ...
  const lines: string[] = [
    `ContextThread Error Log - ${new Date().toISOString()}`,
...
program
  ...
  .action(async (pathArg: string | undefined, options: { index?: boolean; verbose?: boolean }) => {
    ...
    clack.intro('Initializing ContextThread');

    try {
      if (isInitialized(projectPath)) {
        clack.log.warn(`Already initialized in ${projectPath}`);
        clack.log.info('Use "context-thread index" to re-index or "context-thread sync" to update');
        clack.outro('');
        return;
      }
      ...
      if (options.index) {
        ...
        clack.log.info('Run "context-thread index" to index the project');
      }
...
program
  ...
  .action(async (pathArg: string | undefined, options: { force?: boolean }) => {
    ...
    try {
      if (!isInitialized(projectPath)) {
        warn(`ContextThread is not initialized in ${projectPath}`);
        return;
      }
      ...
      if (!options.force) {
        ...
        const answer = await new Promise<string>((resolve) => {
          rl.question(
            chalk.yellow(`${getGlyphs().warn} This will permanently delete all ContextThread data. Continue? (y/N) `),
            resolve
          );
        });
        rl.close();
      ...
      const { default: ContextThread } = await loadContextThread();
      const cg = ContextThread.openSync(projectPath);
      cg.uninitialize();
      ...
      success(`Removed ContextThread from ${projectPath}`);
    } catch (err) {
...
program
  ...
  .action(async (pathArg: string | undefined, options: { force?: boolean; quiet?: boolean; verbose?: boolean }) => {
    ...
    try {
      if (!isInitialized(projectPath)) {
        error(`ContextThread not initialized in ${projectPath}`);
        info('Run "context-thread init" first');
        process.exit(1);
      }
...
program
  ...
  .action(async (pathArg: string | undefined, options: { quiet?: boolean }) => {
    ...
    try {
      ...
      const clack = await importESM('@clack/prompts');
      clack.intro('Syncing ContextThread');
  ...
  .action(async (pathArg: string | undefined, options: { json?: boolean }) => {
    ...
    try {
      if (!isInitialized(projectPath)) {
        ...
        info('Run "context-thread init" to initialize');
        return;
      }
      ...
      if (totalChanges > 0) {
        ...
        info('Run "context-thread sync" to update the index');
      } else {
...
program
  ...
  .action(async (options: {
    ...
    try {
      ...
      if (files.length === 0) {
        info('No files indexed. Run "context-thread index" first.');
        cg.destroy();
        return;
      }
...
/**
 * context-thread serve
 */
program
  .command('serve')
  .description('Start ContextThread as an MCP server for AI assistants')
  ...
  .action(async (options: { path?: string; mcp?: boolean; watch?: boolean }) => {
    ...
    if (options.watch === false) {
      process.env.CONTEXT_THREAD_NO_WATCH = '1';
    }
...
`));
        ...
        console.error(chalk.cyan('  context_thread_search') + '    - Search for code symbols');
        console.error(chalk.cyan('  context_thread_context') + '   - Build context for a task');
        console.error(chalk.cyan('  context_thread_callers') + '   - Find callers of a symbol');
        console.error(chalk.cyan('  context_thread_callees') + '   - Find what a symbol calls');
        console.error(chalk.cyan('  context_thread_impact') + '    - Analyze impact of changes');
        console.error(chalk.cyan('  context_thread_node') + '      - Get symbol details');
        console.error(chalk.cyan('  context_thread_files') + '     - Get project file structure');
        console.error(chalk.cyan('  context_thread_status') + '    - Get index status');
      }
    } catch (err) {
...
program
  ...
  .action(async (pathArg: string | undefined) => {
    ...
    try {
      ...
      const lockPath = path.join(getContextThreadDir(projectPath), 'context-thread.lock');
...
/**
 * context-thread callers <symbol>
 *
 * CLI parity with the MCP graph tools (context_thread_callers/callees/impact) so the
 ...
 *
 * Usage:
 *   git diff --name-only | context-thread affected --stdin
 *   context-thread affected src/lib/components/Editor.svelte src/routes/+page.svelte
 */
program
