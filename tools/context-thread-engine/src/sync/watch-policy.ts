 ...
 *  1. `CONTEXT_THREAD_NO_WATCH=1`    → off  (explicit opt-out always wins)
 *  2. `CONTEXT_THREAD_FORCE_WATCH=1` → on   (overrides auto-detection)
...
export function watchDisabledReason(projectRoot: string, probe: WatchProbe = {}): string | null {
  ...
  if (env.CONTEXT_THREAD_NO_WATCH === '1') {
    return 'CONTEXT_THREAD_NO_WATCH=1 is set';
  }
  if (env.CONTEXT_THREAD_FORCE_WATCH === '1') {
    return null;
  }
