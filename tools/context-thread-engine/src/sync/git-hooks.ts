 ...
 * see watch-policy.ts), the ContextThread index would otherwise go stale until
 * the user runs `context-thread sync` by hand. As an opt-in alternative, we can
 ...
 *
 * The hooks run `context-thread sync` in the background so they never block git,
 * and are guarded by `command -v context-thread` so they no-op cleanly when the
...
const MARKER_BEGIN = '# >>> context-thread sync hook >>>';
const MARKER_END = '# <<< context-thread sync hook <<<';
...
function markerBlock(): string {
  return [
    MARKER_BEGIN,
    '# Keeps the ContextThread index fresh while the live file watcher is off',
    '# (e.g. WSL2 /mnt drives). Runs in the background so it never blocks git.',
    '# Managed by context-thread; remove with `context-thread uninit` or delete this block.',
    'if command -v context-thread >/dev/null 2>&1; then',
    '  ( context-thread sync >/dev/null 2>&1 & ) >/dev/null 2>&1',
    'fi',
    MARKER_END,
  ].join('\n');
}
...
/**
 * Install (or update) the ContextThread sync hooks in a git repository.
...
/**
 * Remove the ContextThread sync hooks. Strips only our marker block; deletes the
...
/** Whether any ContextThread sync hook is currently installed. */
