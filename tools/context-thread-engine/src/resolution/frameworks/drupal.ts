 ...
 *    canonical `hook_X` name, linking implementations to the hook when `context_thread_callers`
 *    is invoked.
 *
 ...
 *   `context_thread_search("form_alter")`. Full hook-node creation (virtual nodes for every hook)
 ...
 * - TODO: Improve hook resolution: create virtual `hook_*` nodes so `context_thread_callers`
