 ...
 *   - `CONTEXT_THREAD_ASCII=1`  -> ASCII (escape hatch for any terminal)
 *   - `CONTEXT_THREAD_UNICODE=1` -> Unicode (opt-in on Windows)
...
export function supportsUnicode(): boolean {
  if (process.env.CONTEXT_THREAD_ASCII === '1') return false;
  if (process.env.CONTEXT_THREAD_UNICODE === '1') return true;
