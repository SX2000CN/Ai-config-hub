 ...
 * @example
 * ```typescript
 * import { FileError, ParseError, setLogger, silentLogger } from 'context-thread';
 *
...
/**
 * Base error class for all ContextThread errors.
 *
 * All ContextThread-specific errors extend this class, allowing you to catch
 * all ContextThread errors with a single catch block.
 *
 * @example
 * ```typescript
 * try {
 ...
 *     console.log(`ContextThread error [${error.code}]: ${error.message}`);
 *   }
 * }
 * ```
 */
 ...
 */
export class FileError extends ContextThreadError {
 ...
 */
export class ParseError extends ContextThreadError {
  readonly filePath: string;
 ...
 */
export class DatabaseError extends ContextThreadError {
 ...
 */
export class SearchError extends ContextThreadError {
 ...
 */
export class VectorError extends ContextThreadError {
  constructor(message: string, operation: string, cause?: Error) {
 ...
 */
export class ConfigError extends ContextThreadError {
...
/**
 * Simple logger for ContextThread operations
 *
...
export const defaultLogger: Logger = {
  debug(message: string, context?: Record<string, unknown>): void {
    if (process.env.CONTEXT_THREAD_DEBUG) {
      console.debug(`[ContextThread] ${message}`, context ?? '');
    }
  },
  warn(message: string, context?: Record<string, unknown>): void {
    console.warn(`[ContextThread] ${message}`, context ?? '');
  },
  error(message: string, context?: Record<string, unknown>): void {
    console.error(`[ContextThread] ${message}`, context ?? '');
  },
};
